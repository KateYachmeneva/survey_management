...
export const wellsSlice = createSlice({
    ...
    selectActiveWell: (state, action) => {
      state.currentWell = [
        ...action.payload.filter(
          (well: IWellData) => well.status_drilling === "ACTV",
        ),
      ][0];
    },
  ...
  extraReducers(builder) {
    ...
    builder.addCase(getAllWells.fulfilled, (state, action) => {
      ...
      state.statusFilteredWell = [
        ...action.payload.sort(sortWells("status_drilling")),
      ];
    });
...
function sortWells(field_name: keyof IWellData) {
  return (a: IWellData, b: IWellData) =>
    a[field_name] > b[field_name] ? 1 : -1;
}
